...
{
    [Fact]
    public void Cart_CreatesNewCart_ForNewUser()
    {
        var dbPath = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid()}.db");
        try
        {
    ...
    [Fact]
    public void CartItem_CanBeAdded_AndRetrieved()
    {
        var dbPath = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid()}.db");
        try
        {
            ...
            using (var connection = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={dbPath}"))
            {
                ...
                cmd.CommandText = @"
                    ...
                    SELECT id, 1, 2 FROM carts WHERE user_id = 1";
                cmd.ExecuteNonQuery();
            }
    ...
    private static void CreateCartDatabase(string dbPath)
    {
        using var connection = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={dbPath}");
        connection.Open();
        using var cmd = connection.CreateCommand();
    ...
    private static void CreateTestUser(string dbPath)
    {
        using var connection = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={dbPath}");
        connection.Open();
        using var cmd = connection.CreateCommand();
    ...
    private static void CreateTestProduct(string dbPath)
    {
        using var connection = new Microsoft.Data.Sqlite.SqliteConnection($"Data Source={dbPath}");
        connection.Open();
        using var cmd = connection.CreateCommand();
